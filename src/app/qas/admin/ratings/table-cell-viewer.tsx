...
type RatingUpdateValues = z.infer<typeof ratingUpdateSchema>
...
export default function TableCellViewer({ item, className }: Props) {
  ...
  const { register, handleSubmit, control, reset, formState: { errors, isSubmitting, isDirty }, } = useForm<RatingUpdateValues>({
    resolver: zodResolver(ratingUpdateSchema),
    values: {
      name: item.name,
  ...
  const onSubmit = async (values: RatingUpdateValues) => {
    ...
    const result = await updateAuditRating(formData)
      ...
      <DrawerContent>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full">
          <DrawerHeader className="gap-1 flex flex-row items-center h-12 justify-between">
            <DrawerTitle>{isEditing ? "Edit Audit Rating" : "Audit Rating Details"}</DrawerTitle>
            <DrawerClose asChild>
